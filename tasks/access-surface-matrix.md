...
| argocd-internal | `argocd.internal.qroksera.com` | internal | `argocd-server` | `argocd` | `https-argocd-internal` | no | yes | no | `Application/argocd-access`<br>`Application/dns-core`<br>`Application/gateway-shared` | `manifests/access/argocd/` | internal publish は CoreDNS-only。Tailnet 向け split DNS には載せない |
...
| harbor-internal | `harbor.internal.qroksera.com` | internal | `harbor-core` / `harbor-portal` | `harbor` | `https-harbor-internal` | no | yes | no | `Application/harbor-access`<br>`Application/dns-core`<br>`Application/gateway-shared` | `manifests/access/harbor/` | internal publish は CoreDNS-only。Cloudflared / Tailscale Split DNS は持たない。runtime automation の到達先は別途 runtime-local endpoint として扱う |
...
| cooklog-internal | `cooklog.internal.qroksera.com` | internal | `cooklog` | `sandbox` | `https-cooklog-internal` | no | yes | yes | `Application/cooklog-access`<br>`Application/dns-core`<br>`Application/dns-tailscale`<br>`Application/gateway-shared` | `manifests/access/cooklog/` | internal route + CoreDNS + Tailnet split DNS の 3 系統管理 |
| api-hub-internal | `api-hub.internal.qroksera.com` | internal | `api-hub` | `sandbox` | `https-api-hub-internal` | no | yes | yes | `Application/api-hub-access`<br>`Application/dns-core`<br>`Application/dns-tailscale`<br>`Application/gateway-shared` | `manifests/access/api-hub/` | internal route と DNS を統合対象。Service 自体は NodePort `32001` も持つ |
| hitomi-upload-viewer-internal | `hitomi-upload-viewer.internal.qroksera.com` | internal | `hitomi-upload-viewer` | `sandbox` | `https-hitomi-upload-viewer-internal` | no | yes | yes | `Application/hitomi-upload-viewer-access`<br>`Application/dns-core`<br>`Application/dns-tailscale`<br>`Application/gateway-shared` | `manifests/access/hitomi-upload-viewer/` | internal route と DNS を統合対象 |
| observability-internal | `observability.internal.qroksera.com` | internal | `victoria-metrics` | `observability` | `https-observability-internal` | no | yes | yes | `Application/observability-access`<br>`Application/dns-core`<br>`Application/dns-tailscale`<br>`Application/gateway-shared` | `manifests/access/observability/` | B-001 初期実装。Grafana / alerting は後続で、外部公開は SSO / 制限方針を決めるまで追加しない |
...
- internal surface の listener / 証明書は hostname ごとに分離し、route / CoreDNS / Tailnet split DNS はそれぞれ `access/**` 配下の shared owner / service owner が持つ
