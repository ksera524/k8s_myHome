...
| observability-internal | `observability.internal.qroksera.com` | internal | `grafana` | `observability` | `https-internal` | no | yes | yes | `Application/observability-access`<br>`Application/dns-core`<br>`Application/dns-tailscale`<br>`Application/gateway-shared` | `manifests/access/observability/` | B-001 設計候補。初期導入では access owner は任意。外部公開は SSO / 制限方針を決めるまで追加しない |
