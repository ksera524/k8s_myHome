...
| observability | - | - | `Application/observability` | `Application/observability-access` | `manifests/platform/observability/` | `manifests/access/observability/` | yes | PH6 後の代替監視基盤。旧 `monitoring` owner は復活させず、新規 owner として設計する。初期導入は VictoriaMetrics + kube-state-metrics と internal access を対象にする |
