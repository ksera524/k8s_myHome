...
| rustfs | `rustfs-app.yaml` (chart `rustfs`) | `rustfs-external-app.yaml` -> `manifests/apps/rustfs` | `bootstrap/applications/platform/rustfs` | `bootstrap/applications/access/rustfs-access` | `Helm chart + values file` | `manifests/access/rustfs/` | yes | 本体と console 公開を分離。`sandbox-config` が `rustfs-svc` に依存 |
