...
log_status "libvirt sudo権限設定を開始します"
log_status "設定対象ユーザー: $CURRENT_USER"

# sudoersファイル作成
SUDOERS_FILE="/etc/sudoers.d/50-libvirt-nopasswd"
TEMP_FILE="/tmp/50-libvirt-nopasswd.tmp"

log_status "sudoers設定ファイルを作成中..."
...
if sudo visudo -c -f "$TEMP_FILE"; then
    log_status "sudoers設定の文法チェック完了"
    # sudoersディレクトリにコピー
    sudo cp "$TEMP_FILE" "$SUDOERS_FILE"
    sudo chmod 440 "$SUDOERS_FILE"
    log_status "✓ sudoers設定ファイルをインストールしました: $SUDOERS_FILE"
else
    log_error "sudoers設定の文法エラーが検出されました"
    rm -f "$TEMP_FILE"
    exit 1
fi
...
if ! groups "$CURRENT_USER" | grep -q libvirt; then
    log_warning "ユーザー $CURRENT_USER をlibvirtグループに追加します"
    sudo usermod -aG libvirt "$CURRENT_USER"
    log_warning "グループの変更を反映するには、一度ログアウトして再ログインが必要です"
fi

# テスト実行
log_status "設定をテスト中..."
if sudo -n virsh list --all >/dev/null 2>&1; then
    log_status "✓ virshコマンドのパスワードなし実行: OK"
else
    log_warning "virshコマンドのテスト失敗（再ログインが必要な可能性があります）"
fi

log_status "=== libvirt sudo権限設定完了 ==="
log_status ""
log_status "次のステップ:"
log_status "1. 一度ログアウトして再ログインしてください（グループ変更の反映）"
log_status "2. make all を実行してください"
log_status ""
log_status "設定を元に戻す場合:"
log_status "  sudo rm $SUDOERS_FILE"