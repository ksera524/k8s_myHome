...
get_harbor_credentials() {
    ...
    if ssh -o StrictHostKeyChecking=no k8suser@192.168.122.10 "kubectl get secret $secret_name -n $namespace" >/dev/null 2>&1; then
        ...
        if [[ -n "$HARBOR_USERNAME" && -n "$HARBOR_PASSWORD" ]]; then
            ...
            log_status "✓ ESO管理のHarbor認証情報取得完了"
            log_debug "HARBOR_USERNAME: $HARBOR_USERNAME"
            log_debug "HARBOR_PASSWORD: ${HARBOR_PASSWORD:0:3}... (先頭3文字のみ表示)"
            return 0
        else
            log_error "K8s SecretからのHarbor認証情報取得に失敗"
            return 1
        fi
    else
        log_warning "$secret_name Secret ($namespace) が見つかりません"
        return 1
    fi
}
...
get_github_credentials() {
    ...
    log_debug "GitHub認証情報をK8s Secret($namespace/$secret_name)から取得中..."
    
    if ssh -o StrictHostKeyChecking=no k8suser@192.168.122.10 "kubectl get secret $secret_name -n $namespace" >/dev/null 2>&1; then
        log_debug "ESO管理のGitHub認証情報を取得中..."
        ...
        if [[ -n "$GITHUB_TOKEN" && -n "$GITHUB_USERNAME" ]]; then
            ...
            log_status "✓ ESO管理のGitHub認証情報取得完了"
            log_debug "GITHUB_USERNAME: $GITHUB_USERNAME"
            log_debug "GITHUB_TOKEN: ${GITHUB_TOKEN:0:8}... (先頭8文字のみ表示)"
            return 0
        else
            log_error "K8s SecretからのGitHub認証情報取得に失敗"
            return 1
        fi
    else
        log_warning "$secret_name Secret ($namespace) が見つかりません"
...
check_argocd_app_sync() {
    ...
    log_debug "$app_name アプリケーションの同期状態確認中..."
    ...
    if [[ "$sync_status" == "Synced" ]]; then
        log_status "✓ $app_name: 同期済み"
        return 0
    else
        log_warning "$app_name: 同期状態 = $sync_status"
        return 1
    fi
}
...
force_sync_argocd_app() {
    ...
    log_debug "$app_name アプリケーション強制同期中..."
...
check_external_secret_sync() {
    ...
    log_debug "$secret_name ExternalSecret同期状態確認中..."
    ...
    if [[ "$status" == "True" ]]; then
        log_status "✓ $secret_name: ExternalSecret同期済み"
        return 0
    else
        log_warning "$secret_name: ExternalSecret同期状態 = $status"
        return 1
    fi
}
...
check_k8s_secret_exists() {
    ...
    if ssh -o StrictHostKeyChecking=no k8suser@192.168.122.10 \
        ...
        log_debug "✓ K8s Secret $secret_name ($namespace) 存在確認"
        return 0
    else
        log_debug "✗ K8s Secret $secret_name ($namespace) が見つかりません"
        return 1
    fi
}
...
validate_harbor_setup() {
    ...
    if ! ssh -o StrictHostKeyChecking=no k8suser@192.168.122.10 'kubectl get namespace harbor' >/dev/null 2>&1; then
        log_error "Harbor namespaceが見つかりません"
        return 1
    fi
    ...
    if [[ "$ready_pods" -gt 0 ]]; then
        log_status "✓ Harbor Pods稼働中: $ready_pods pods"
    else
        log_warning "Harbor Podsが稼働していません"
        return 1
    fi
    
    # 3. Harbor認証情報確認
    if get_harbor_credentials; then
        log_status "✓ Harbor認証情報取得成功"
    else
        log_error "Harbor認証情報取得失敗"
        return 1
    fi
    
    log_status "✓ Harbor設定完全性確認完了"
    return 0
}

# External Secrets Operator設定確認
validate_eso_setup() {
    log_status "External Secrets Operator設定確認中..."
    
    # 1. ESO Namespace確認
    if ! ssh -o StrictHostKeyChecking=no k8suser@192.168.122.10 'kubectl get namespace external-secrets-system' >/dev/null 2>&1; then
        log_error "external-secrets-system namespaceが見つかりません"
        return 1
    fi
    ...
    if [[ "$eso_ready" -gt 0 ]]; then
        log_status "✓ External Secrets Operator稼働中: $eso_ready pods"
    else
        log_error "External Secrets Operatorが稼働していません"
        return 1
    fi
    ...
    if ssh -o StrictHostKeyChecking=no k8suser@192.168.122.10 \
        ...
        log_error "ClusterSecretStore (pulumi-esc-store) が見つかりません"
        return 1
    fi
    
    log_status "✓ External Secrets Operator設定確認完了"
    return 0
}

log_debug "共通K8sユーティリティライブラリ読み込み完了"