...
if [[ $? -eq 0 ]]; then
    ...
    echo ""
    echo "=== 次のステップ ==="
    echo "1. cloud-init完了を待機: ssh k8suser@192.168.122.10 'sudo cloud-init status --wait'"
    echo "2. k8s構築: cd ../ansible && ansible-playbook playbook.yml"
    echo ""
    echo "=== 確認コマンド ==="
    echo "VM状態: sudo virsh list --all"
    echo "ネットワーク: sudo virsh net-dhcp-leases default"
    echo "ログ確認: sudo journalctl -u libvirtd -f"
