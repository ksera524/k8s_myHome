...
# MetalLB + Ingress Controller + cert-manager + Harbor
...
# 詳細状態表示
print_status "=== 詳細状態 ==="
ssh -o StrictHostKeyChecking=no k8suser@192.168.122.10 'kubectl get pods --all-namespaces | grep -E "(metallb|ingress|cert-manager)"'

echo ""
echo "=== 次のステップ ====" 
echo "1. アプリケーション移行: Phase 5でアプリケーションをk8sに移行"
echo "2. Ingress設定: HTTP/HTTPSアクセス用のIngress設定"
echo "3. 証明書設定: cert-managerでTLS証明書取得"
echo ""
